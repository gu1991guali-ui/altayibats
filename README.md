...
بعد رفع نسخة عداد الزيارات، شغّل الملف التالي مرة واحدة من Supabase SQL Editor:

```sql
database/02_ADD_VISIT_ANALYTICS.sql
```

بعد نجاح التشغيل افتح `/admin` واضغط **تحديث العداد**.  
العداد يسجل الصفحات العامة فقط، ولا يحتسب `/admin` أو صفحات تسجيل الدخول.
