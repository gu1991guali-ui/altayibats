...
export function VideoDetailsPage() {
  ...
  async function handleAddComment(event: FormEvent<HTMLFormElement>) {
    ...
    if (!content) {
      setError("اكتب تعليقًا قبل الإرسال.");
      return;
    }
  ...
  if (authLoading || isLoading) {
    return <div className="empty-state">جار تحميل الفيديو...</div>;
  }

  if (!video) {
    return <StatusMessage tone="error">{error || "لم يتم العثور على الفيديو."}</StatusMessage>;
  }
    ...
    <section className="video-detail-layout">
      ...
      <div
        className={`player-wrap ${
          isShortVideo ? "player-wrap-short aspect-[9/16]" : "player-wrap-long aspect-video"
        }`}
      >
        <video src={videoUrl} controls preload="metadata" />
      </div>

      <div className="content-panel video-detail-header">
        <h1>{video.title}</h1>
